...
* webpack.dev.js
  ...
  这样就可以配置热更新，按照目前的配置，修改css文件时，是可以实现热更新时。

  

  但我们修改任意js文件时，是不进行热更新的。

  

  原因是：**我们必须配置，哪些文件是要进行热更新，配置外的文件是执行自动刷新的**
  ...
  当我们修改 `math.js`文件时，则进行热更新，修改以外的js文件则进行自动刷新
...
背景：前端框架如 `vue` `react`, 体积大，其特点是较稳定，不常升级的版本。在开发环境下每次`npm run dev`，及会重复打包 `vue`、`react` 包 
