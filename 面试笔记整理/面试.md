...
* Safari: **webkit** 内核 Opera:以前是 presto 内核，Opera 现已改用 Google Chrome 的 Blink 内核 
...
那么浏览器会识别该文档为 css 文件，就会<font color="red">并行下载资源并且不会停止对当前文档的处理</font>。 这也是为什么建议使用 link 方式来加载 css，而不是使用@import 方式。
...
23、行内元素和块级元素的具体区别是什么？



24、什么是外边距重叠，重叠的结果是什么？如何触发 BFC
